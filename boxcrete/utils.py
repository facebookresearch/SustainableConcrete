...
def load_concrete_strength(
    ...
    # 3-class ``{0, 1, 2}`` v5 schema. Older 2-class data and synthetic
    # test data with non-canonical names skip the check silently.
    if mix_name_column in df.columns and "Material Source" in df.columns:
        ...
        except Exception:  # pragma: no cover - mix_naming import is always available
                ...
                if (
                    ...
                    for _, row in (
                        ...
                        try:
                            ...
                            Exception
                        ):  # pragma: no cover - names already regex-validated
                            continue
                    ...
                    if mismatches:
                        raise ValueError(
                            ...
                            "recorded class): "
                            f"{mismatches}. The Material Source column in "
                            "data/boxcrete_data.csv must agree with the "
                            "canonical mix name."
                        )
