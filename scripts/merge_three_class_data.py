...
NEW_FILE_CSV = REPO_ROOT / "data" / "Meta Main File_Updated.xlsx - ForGitHub.csv"
...
def _build_canonical_name_map(new_file_df: pd.DataFrame) -> dict[str, str]:
    ...
    mix_any_strength = new_file_df.groupby("Mix Name")["_row_has_strength"].any()
    strengthless_mortar_names = sorted(
        new_file_df[new_file_df["Mortar or Concrete"] == "Mortar"]["Mix Name"]
        .dropna()
        .unique()
        .tolist()
    )
    ...
    if available_clay_cols:
        clay_row_mask = new_file_df[available_clay_cols].fillna(0).gt(0).any(axis=1)
    ...
    new_file_df = new_file_df[~new_file_df["Mix Name"].isin(excluded_mix_names)].copy()
    ...
    mortar_names = _canonical_order_for_class(
        sorted(
            new_file_df[new_file_df["_class"] == CLASS_MORTAR_SET1]["Mix Name"].unique()
        ),
        CLASS_MORTAR_SET1,
    )
...
def _build_legacy_to_canonical_table(
    ...
    # builder, but be safe).
    nf = new_file_df[~new_file_df["Mix Name"].isin(strengthless_mortar_names)].copy()
...
def main() -> None:
    if not PRE_V5_FIXTURE.exists():
        ...
        print(
            f"[snapshot] {CURRENT_CSV.name} -> {PRE_V5_FIXTURE.relative_to(REPO_ROOT)}"
        )

    pre_v5 = _load_pre_v5()
    new = _load_new_file()
    print(
        f"[load] pre-v5: {len(pre_v5)} rows, "
        f"{pre_v5['Mix Name'].nunique()} unique mixes"
    )
    ...
    print(
        f"[strength-less] {len(strengthless_mortar_names)} mortars dropped: "
        f"{strengthless_mortar_names}"
    )
    ...
    class_dist = (
        out_df.drop_duplicates("Mix Name")["Material Source"].value_counts().to_dict()
    )
    print(
