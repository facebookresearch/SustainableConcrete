...
def main() -> int:
    ...
    by_class = df.drop_duplicates("Mix Name")["Material Source"].value_counts().to_dict()
    _check(
        "mortar_count",
        by_class.get(CLASS_MORTAR, 0) == EXPECTED_MORTAR_COUNT,
        f"mortars (MS=0) = {by_class.get(CLASS_MORTAR, 0)} (expected {EXPECTED_MORTAR_COUNT})",
    )
    _check(
        "set2_count",
        by_class.get(CLASS_SET2, 0) == EXPECTED_SET2_COUNT,
        f"Set 2 (MS=1) = {by_class.get(CLASS_SET2, 0)} (expected {EXPECTED_SET2_COUNT})",
    )
    _check(
        "set3_count",
        by_class.get(CLASS_SET3, 0) == EXPECTED_SET3_COUNT,
        f"Set 3 (MS=2) = {by_class.get(CLASS_SET3, 0)} (expected {EXPECTED_SET3_COUNT})",
    )
    ...
    for mix_name, sub in df.groupby("Mix Name"):
        ...
        if mix_name.startswith("M"):
            if cls != CLASS_MORTAR:
                ...
                print(f"  [FAIL] M_prefix_class: {mix_name} has class {cls} (expected 0)")
                break
    ...
    _check(
        "M_in_range",
        not out_of_range_M,
        f"M-canonicals must be M1..M{EXPECTED_MORTAR_COUNT}; out-of-range: {out_of_range_M}",
    )
    ...
    out_of_range_C = sorted(
        [
            n
            for n in df["Mix Name"].unique()
            if (m := re.match(r"^C(\d+)$", n))
            and not (1 <= int(m.group(1)) <= max_C)
        ]
    )
    _check(
        "C_in_range",
    ...
    table_class = {r["canonical_name"]: r["material_class"] for _, r in table.iterrows()}
    mismatches = []
    ...
    mixes_no_strength = (
        df.groupby("Mix Name")["_has_strength"].any().pipe(lambda s: s[~s].index.tolist())
    )
