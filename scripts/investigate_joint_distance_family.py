...
"""Comprehensive joint-distance kernel family ablation.

After ``joint_hamming_matern`` showed a strong in-distribution
improvement (LOO 495, bLOO 717, PIT-KS 0.030 — best calibration on
v5), this script tests the full joint-distance kernel family to
identify the strongest variant for production:

Variants:
    joint_hamming_matern              (already winning; baseline reference)
    joint_chain_matern                (uses (i - j)^2 categorical distance)
    joint_hamming_matern_nu05         (Matern smoothness 0.5)
    joint_hamming_matern_nu25         (Matern smoothness 2.5)
    joint_embedding_matern_d1         (learnable per-class scalar; sibling
                                       of legacy_continuous_ard)
    joint_embedding_matern_d2         (learnable 2D embedding)
    joint_embedding_matern_d3         (learnable 3D embedding)
    joint_embedding_matern_d1_linear_init  (legacy-style integer init)
    legacy_continuous_ard             (ground-truth bLOO target)
    hamming                           (categorical-product baseline)
    rbf_embedding_d2                  (current production candidate)

Per cell: in-distribution v5 fit + 3 LOCO holdouts × 3 seeds.
11 variants × 4 contexts × 3 seeds = 132 cells. Estimated ~3.5 hours
wall time at ~90s per fit (longer if the older grids contend for CPU).

Output:
    experiments/JOINT_DISTANCE_FAMILY_ABLATION.md
    experiments/joint_distance_family_ablation.csv
"""
...
WRITEUP = (
    REPO_ROOT / "experiments" / "JOINT_DISTANCE_FAMILY_ABLATION.md"
)
RESULTS_CSV = (
...
VARIANTS = [
    ...
    # Established baselines for direct comparison
...
def main() -> int:
    ...
    out: list[str] = []
    out.append("# Joint-distance kernel family — comprehensive ablation")
    out.append("")
    out.append(
        "Identifies the best variant in the joint-distance kernel "
        "family (kernels of the form $K = M_\\nu(\\sqrt{d^2_\\text{feat} "
        "+ d^2_\\text{cat}})$) and compares against the established "
        "baselines (`hamming`, `legacy_continuous_ard`, `rbf_embedding_d2`)."
    )
    out.append("")
    ...
    for hc in [0, 1, 2]:
        ...
        if len(sub) == 0:
            continue
        out.append(f"## LOCO class {hc} held out (mean ± std across 3 seeds)")
        out.append("")
