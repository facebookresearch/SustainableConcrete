    ...
    *,
    ...
    def _fit(X, Y, Yvar, X_bounds, seed):
        ...
        def _patched_factory(*args, **kwargs):
            kwargs.setdefault(
                "include_lognormal_baseline", include_lognormal_baseline
            )
...
VARIANTS = {
    ...
    "stage2a_v5_hamming_bare": {
        ...
        "data": "v5",
        "description": (
            "v5 + Hamming + bare. Kernel × data cell (Hamming, v5)."
        ),
        "stage": 2,
    },
    # ------------------------------------------------------------
...
def _lengthscale_diagnostics(model) -> dict[str, float]:
    ...
    try:
        ...
        try:
            ...
            if len(binder_ls) > 1:
                out["binder_within_ratio"] = (
                    max(binder_ls) / max(min(binder_ls), 1e-12)
                )
        except ValueError:
            pass
        try:
            ...
            if len(agg_ls) > 1:
                out["aggregate_within_ratio"] = (
                    max(agg_ls) / max(min(agg_ls), 1e-12)
                )
        except ValueError:
            pass
...
def _bloo_set12_metrics(model, n_real: int) -> dict[str, float]:
    ...
    log_pd = -0.5 * (err / std_v).pow(2) - std_v.log() - 0.5 * math.log(
        2 * math.pi
    )
...
def _run_class_holdout(
    ...
    for vid in variants:
        ...
        for seed in seeds:
            ...
            rows.append(row)
            pd.DataFrame(rows).to_csv(
                RESULTS_CSV.with_suffix(
                    f".class{holdout_class}_holdout.partial.csv"
                ),
                index=False,
            )
    return pd.DataFrame(rows)
...
def _headline_analysis(df_main: pd.DataFrame, _df_set3: pd.DataFrame) -> str:
    ...
    lines.append(
        "## Honest no-regression check vs the currently-deployed model"
    )
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("| Metric | pre-v5 + legacy V2 (deployed) | v5 + legacy V2 | v5 + Hamming bare (proposed) |")
    lines.append("|---|---|---|---|")
    ...
    lines.append("")
    lines.append(
        "**Both choices have merit. The right call depends on what we "
        "weight more:**"
    )
    lines.append("")
    lines.append(
...
def write_markdown(
    ...
    if df_set3 is not None and not df_set3.empty:
        lines.append(
            "## Held-out Set-3 (fit on Sets 1+2 only; evaluate on Set 3)"
        )
        lines.append("")
        lines.append(
    ...
    if df_class_holdout is not None and not df_class_holdout.empty:
        ...
        for c in sorted(agg_holdout["holdout_class"].unique()):
            ...
            lines.append("")
            lines.append(
                "| Variant | RMSE (psi) | MLPD | PIT-KS | cov95 | seeds |"
            )
            lines.append("|---|---|---|---|---|---|")
...
def main() -> int:
    ...
    if args.variants == "all":
        variants = sorted(
            VARIANTS.keys(), key=lambda k: (VARIANTS[k]["stage"], k)
        )
    else:
    ...
    if args.class_holdouts:
        ...
        holdout_classes = [
            int(c.strip())
            for c in args.class_holdouts.split(",")
            if c.strip()
        ]
        all_holdout = []
        ...
        df_set3_existing = combined[
            combined["data"] == "v5_set3_heldout"
        ]
        df_holdout_all = combined[
            combined["data"].astype(str).str.contains("class\\d_heldout", regex=True, na=False)
        ]
        write_markdown(
            df_main_existing, df_set3_existing, WRITEUP_MD,
