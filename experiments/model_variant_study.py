...
def _build_source_aware_kernel(d_in: int, source_dim: int) -> torch.nn.Module:
    ...
    # IndexKernel(rank=0) = pure diagonal δ(s_i, s_j) (after var clamp); 2 sources.
...
def _make_b_double_prime_time_gated_builder(gate_tau: float = 0.05, gate_learnable: bool = False):
    ...
    """
    if gate_learnable and not getattr(_make_b_double_prime_time_gated_builder, "_warned", False):
        import warnings
